...
{
    public class PostgresqlCreateFunctionTask: Task
    {
        [Required]
                ...
                foreach (ITaskItem item in ProcedureScripts)
                {
                    if (item.ItemSpec.Length > 0)
                    {
                        try
                        {
                            ...
                            PostgresqlScriptResult result = PostgresqlScriptRunner.RunStatements(scriptBody, this.ConnectionString, null);
