...
{
    public class OracleCreateSequenceTask: Task
    {
        [Required]
        public ITaskItem[] SequenceScripts { get; set; }
        ...
        public override bool Execute()
        {
            ...
            if (SequenceScripts != null)
            {
                foreach (ITaskItem item in SequenceScripts)
                {
                    if (item.ItemSpec.Length > 0)
                    {
                        try
                        {
                            ...
                            OracleScriptResult result = OracleScriptRunner.RunStatements(scriptBody, this.ConnectionString);
